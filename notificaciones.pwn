...
#define 	TD_NOTIFICATION_LETTER_X 0.179166
#define 	TD_NOTIFICATION_LETTER_Y 1.100000

#define COLOR_KORNBOX 0xC246D6ff
...
new TextDrawsNotification[MAX_PLAYERS][3][InformationTDN],
...
public SendNotification(playerid, const reason[])
{
    for(new cycle; cycle < 3; cycle++)
    {
        if(!TextDrawsNotification[playerid][cycle][Use])
        {
            ...
            SetTimerEx("TimerHideNotification", 7000, false, "i", playerid);
...
stock MinPosYNotification(playerid, TDN)
{
    if(TDN == 0)
    {
        TextDrawsNotification[playerid][TDN][MinPosY] = PI[playerid][pCargos] == 0 ? 130.000000 : 153.0;
    }
    else
    {
...
stock CreateTDN(playerid, TDN)
{
    if(TextDrawsNotification[playerid][TDN][Use] == 1)
    {
		...
		TextDrawsNotification[playerid][TDN][TextDraw] = CreatePlayerTextDraw(playerid, 490.000000, TextDrawsNotification[playerid][TDN][MinPosY], TextDrawsNotification[playerid][TDN][Text]);
        ...
        PlayerTextDrawTextSize(playerid, TextDrawsNotification[playerid][TDN][TextDraw], 636.000000, 1.000000);
        ...
        PlayerTextDrawBackgroundColor(playerid, TextDrawsNotification[playerid][TDN][TextDraw], 0x0000005A);
        PlayerTextDrawBoxColor(playerid, TextDrawsNotification[playerid][TDN][TextDraw], COLOR_KORNBOX);
